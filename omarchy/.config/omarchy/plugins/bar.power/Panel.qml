...
Panel {
  ...
  // 0 = power profile row, 1 = charge limit row
  property int cursorRow: 0
  // Battery charge end threshold, read back from the hardware.
  property int chargeLimit: 0
  property int limitIndex: 0
  readonly property var chargeLimitOptions: [60, 80, 95, 100]
  ...
  function selectLimitByDelta(delta) {
    limitIndex = Model.clampIndex(limitIndex + delta, chargeLimitOptions.length)
  }

  function syncLimitIndex() {
    var idx = chargeLimitOptions.indexOf(chargeLimit)
    if (idx >= 0) limitIndex = idx
  }
  ...
  function updateChargeLimit(raw) {
    ...
    chargeLimit = value
    if (!cursorActive) syncLimitIndex()
  }

  function setChargeLimit(value) {
    if (!value || limitProc.running) return
    limitProc.command = ["battery-charge-limit", String(value)]
  ...
  Process {
    id: limitReadProc
    command: ["battery-charge-limit"]
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.updateChargeLimit(text) }
  }

  // Applying a limit is asynchronous in the firmware; re-read once it exits.
  Process {
    id: limitProc
    onExited: root.refresh()
  }
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      onMoveRequested: function(dx, dy) {
        ...
        if (dy !== 0) root.cursorRow = Model.clampIndex(root.cursorRow + dy, 2)
        else if (dx !== 0) {
          if (root.cursorRow === 0) root.selectProfileByDelta(dx)
          else root.selectLimitByDelta(dx)
        }
      }
      onActivateRequested: {
        if (!root.cursorActive) return
        if (root.cursorRow === 0) root.activateSelectedProfile()
        else root.setChargeLimit(root.chargeLimitOptions[root.limitIndex])
      }
      ...
      Column {
          ...
          Row {
            ...
            Repeater {
              ...
              Button {
                ...
                hasCursor: root.cursorActive && root.cursorRow === 0 && root.profileIndex === index
        ...
        // ---------- Charge limit picker ----------
        // Caps how full the battery charges; lower limits extend cell life.
        Column {
          ...
          Row {
            ...
            Repeater {
              ...
              Button {
                ...
                hasCursor: root.cursorActive && root.cursorRow === 1 && root.limitIndex === index
