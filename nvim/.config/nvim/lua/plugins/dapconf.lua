...
dap.listeners.after['event_debugpyAttach']['dap-python'] = function (_, config)
  ...
  local on_connect = function (_)
  end
  local session = require('dap.session'):connect({host = config.connect.host, port = config.connect.port}, {}, on_connect)
...
dap.configurations.python = {
  {
    ...
    cwd = "${workspaceFolder}/test";
    args = function ()
      return {"on-demand", "--current-cluster", "--skip-init", vim.fn.input("Please supply the name of the function: "), "${file}"}
    end;
    subProcess = true;
  },
}
