...
M.misc = function()
   ...
   -- lazygit
   map("n", miscMap.open_lazygit, ":lua _LAZYGIT_TOGGLE()<CR>", opt)
...
M.dap = function ()
  ...
  map("n", m.set_conditional_breakpoint, ":lua require'dap'.set_breakpoint(vim.fn.input('Breakpoint condition: '))<CR>", opt)
  map("n", m.set_logpoint_message, ":lua require'dap'.set_breakpoint(nil, nil, vim.fn.input('Log point message: '))<CR>", opt)
  map("n", m.repl_open, ":lua require'dap'.repl.open()<CR>", opt)
  map("n", m.run_last, ":lua require'dap'.run_last()<CR>", opt)
  map("n", m.go_test_debug, ":lua require'dap-go'.debug_test()<CR>", opt)
  map("n", m.eval_expression, ":lua require'dapui'.eval()<CR>", opt)
  map("n", m.ui_toggle, ":lua require'dapui'.toggle()<CR>", opt)
  map("n", m.sonic_debug_test, ":lua require'plugins.dapconf'.trigger_sonic()<CR>", opt)
end
...
M.coc = function()
    local m = user_map.coc
    local plugopts = {silent = false, noremap = false}

    map("n", m.definition, "<Plug>(coc-definition)", vim.tbl_extend("force", opt, plugopts))
    map("n", m.type_definition, "<Plug>(coc-type-definition)", vim.tbl_extend("force", opt, plugopts))
    map("n", m.references, ":lua require'telescope'.extensions.coc.references_used(require'telescope.themes'.get_cursor({path_display = {'shorten'}, layout_config={width=150}}))<CR>", opt)
    map("n", m.implementation, ":lua require'telescope'.extensions.coc.implementations(require'telescope.themes'.get_cursor({path_display = {'shorten'}, layout_config={width=150}}))<CR>", opt)
    map("n", m.goto_test, ":CocCommand go.test.toggle<CR>", opt)

    map("n", m.diagnostic_prev, "<Plug>(coc-diagnostic-prev)", vim.tbl_extend("force", opt, plugopts))
    map("n", m.diagnostic_next, "<Plug>(coc-diagnostic-next)", vim.tbl_extend("force", opt, plugopts))
    map("n", m.rename, "<Plug>(coc-rename)", vim.tbl_extend("force", opt, plugopts))
    map("n", m.format_selected, "<Plug>(coc-format-selected)", vim.tbl_extend("force", opt, plugopts))
    map("x", m.format_selected, "<Plug>(coc-format-selected)", vim.tbl_extend("force", opt, plugopts))
    map("n", m.diagnostics, ":CocDiagnostics<CR>", opt)
end

M.telescope_coc = function()
    local m = user_map.telescope_coc

    map("n", m.document_symbols, ":Telescope coc document_symbols<CR>")
    map("n", m.workspace_symbols, ":Telescope coc workspace_symbols<CR>")
end

M.lsp = function()
    local m = user_map.telescope_coc

    map("n", m.document_symbols, ":Telescope lsp_document_symbols<CR>")
...
M.hop = function ()
  ...
  map("n", m.jump_word, ":HopWord<CR>", opt)
end
